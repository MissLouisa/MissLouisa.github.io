...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document
    .getElementById('buttonSortSection')
    .addEventListener('click', function() {
  ...
  document.getElementById('buttonDelete').addEventListener('click', function() {
    ...
    fetch('users/deleteRE/' + deleteTitle, {
      ...
      ) // wait for data from server to be valid
...
function createList() {
  ...
  reArray.forEach(function(element) {
    ...
    li.innerHTML =
      "<a data-transition='pop' class='oneRE' data-parm=" +
      element.Title +
      "  href='#page5'>Get Details </a> " +
      element.Title +
      '  ' +
      element.Section;
...
function FillArrayFromServer() {
  ...
  fetch('/users/REList')
    .then(function(theResonsePromise) {
      // wait for reply.  Note this one uses a normal function, not an => function
...
function addNewRE(newRE) {
  ...
  fetch(request)
    ...
    .then(
      theResonsePromiseJson => console.log(theResonsePromiseJson),
